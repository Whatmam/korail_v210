...
#define BOOTLOADER_SW_VERSION		"ver 1.4.1(Ramdisk)" 	// 2014.05.07 jay solidtec korail software version 
...
static void board_version_view(void)
{
#if 0
	printf("\r\n");
	printf("\r\n");
...
#endif
	printf("\r\n");
	printf("               ** Solidtek Bootloader %s **                \r\n", BOOTLOADER_SW_VERSION);
	printf("\r\n");	
}

static void s5pc11x_msleep (int count)
{
	int i;
...
#if KORAIL_BASE
//2014.05.07 JAY Change for korail - SROM_nCS5 USE.
#if defined(DM9000_16BIT_DATA)
	SROM_BW_REG &= ~(0xf << 20);
	SROM_BW_REG |= (0<<23) | (1<<22) | (1<<21) | (1<<20);

#else
	SROM_BW_REG &= ~(0xf << 20);
	SROM_BW_REG |= (1<<23) | (0<<22) | (1<<21) | (0<<20);
#endif
	...
	MP01CON_REG = tmp;
#else
//2013.08.21 JAY Change for Mediana - SROM_nCS4 USE.
#if defined(DM9000_16BIT_DATA)
	SROM_BW_REG &= ~(0xf << 16);
	SROM_BW_REG |= (0<<19) | (1<<18) | (1<<17) | (1<<16);

#else
	SROM_BW_REG &= ~(0xf << 16);
	SROM_BW_REG |= (1<<19) | (0<<18) | (1<<17) | (0<<16);
#endif
...
int board_init(void)
{
	...
	TCON_REG &= ~(0xf << 0); // 2014.05.07 jay Booting Buzzer stop
...
int dram_init(void)
{
	...
	for(i = 0; i < CONFIG_NR_DRAM_BANKS; i++){/* SDKIM */
