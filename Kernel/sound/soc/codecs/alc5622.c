	...
	unsigned int value)
{
    ...
    {
        printk("[MMM] ALC5622 write Fail reg=%x,value=%x\n",reg,value);
        return -EIO;
    }
    else
    {
				 ...
				 int div_id, int div)
{
	...
	printk("[MMM] wm8580_set_dai_clkdiv\r\n");
...
static int wm8580_register(struct wm8580_priv *wm8580)
{
	...
	wm8580_write(codec, 0x02, 0x0000);
    wm8580_write(codec, 0x04, 0x1010);
    wm8580_write(codec, 0x06, 0x8080);
    wm8580_write(codec, 0x08, 0xe808);
    wm8580_write(codec, 0x0a, 0xe808);
    wm8580_write(codec, 0x0c, 0x0000);
    wm8580_write(codec, 0x0e, 0x1f1f);

    wm8580_write(codec, 0x10, 0xe0f0);
    wm8580_write(codec, 0x12, 0xe78f);
    wm8580_write(codec, 0x14, 0x7f5f);

    wm8580_write(codec, 0x16, 0x0009);
    wm8580_write(codec, 0x1c, 0x0700);

    wm8580_write(codec, 0x22, 0x0100);


    wm8580_write(codec, 0x34, 0x8000);
    wm8580_write(codec, 0x36, 0x066f);
    wm8580_write(codec, 0x38, 0x0000);
    wm8580_write(codec, 0x3a, 0x8820);
    wm8580_write(codec, 0x3c, 0xe7ff);

    wm8580_write(codec, 0x3e, 0x9601);
    wm8580_write(codec, 0x40, 0x5330);
    wm8580_write(codec, 0x42, 0x0000);
    wm8580_write(codec, 0x44, 0x0000);

    wm8580_write(codec, 0x4a, 0x0000);
    wm8580_write(codec, 0x4c, 0x1c0e);
    wm8580_write(codec, 0x4e, 0x1c0e);
    wm8580_write(codec, 0x50, 0x0000);
    wm8580_write(codec, 0x52, 0x0000);
    wm8580_write(codec, 0x54, 0x0002);
    wm8580_write(codec, 0x56, 0x0000);
    wm8580_write(codec, 0x58, 0x0030);
    wm8580_write(codec, 0x5a, 0x0000);
    wm8580_write(codec, 0x5e, 0x0000);
    wm8580_write(codec, 0x60, 0x0497);
    wm8580_write(codec, 0x62, 0x0000);
    wm8580_write(codec, 0x66, 0x0000);
    wm8580_write(codec, 0x68, 0x000b);
    wm8580_write(codec, 0x6a, 0x0046);
    wm8580_write(codec, 0x6c, 0x0000);
    wm8580_write(codec, 0x7c, 0x10ec);
    wm8580_write(codec, 0x7e, 0x2203);
    wm8580_write(codec, 0x6a, 0x0039);
    wm8580_write(codec, 0x6c, 0x9000);
...
//
//*****************************************************************************
int rt5621_AudioOutEnable(struct snd_soc_codec *codec, unsigned short int WavOutPath, bool Mute)
{
    int RetVal=0;
    ...
    if(Mute)
    {
        switch(WavOutPath)
        {
            case RT_WAVOUT_ALL_ON:
                RetVal = rt5621_write_mask(codec, RT5621_SPK_OUT_VOL,
                            RT_L_MUTE|RT_R_MUTE, RT_L_MUTE|RT_R_MUTE);  //Mute Speaker right/left channel
                RetVal = rt5621_write_mask(codec, RT5621_HP_OUT_VOL,
                            RT_L_MUTE|RT_R_MUTE, RT_L_MUTE|RT_R_MUTE);  //Mute headphone right/left channel
                RetVal = rt5621_write_mask(codec, RT5621_MONO_AUX_OUT_VOL,
                            RT_L_MUTE|RT_R_MUTE, RT_L_MUTE|RT_R_MUTE);  //Mute Aux/Mono right/left channel
                RetVal = rt5621_write_mask(codec, RT5621_STEREO_DAC_VOL,
            ...
            case RT_WAVOUT_AUXOUT:
                RetVal = rt5621_write_mask(codec, RT5621_MONO_AUX_OUT_VOL,
                            RT_L_MUTE|RT_R_MUTE, RT_L_MUTE|RT_R_MUTE);  //Mute AuxOut right/left channel
                break;
    ...
    else
    {
        switch(WavOutPath)
        {
            case RT_WAVOUT_ALL_ON:
                ...
                RetVal = rt5621_write_mask(codec, RT5621_STEREO_DAC_VOL, 0,
                            RT_M_HP_MIXER|RT_M_SPK_MIXER|RT_M_MONO_MIXER); //Mute DAC to HP,Speaker,Mono Mixer
                break;
            ...
            case RT_WAVOUT_DAC:
                RetVal = rt5621_write_mask(codec, RT5621_STEREO_DAC_VOL, 0,
                            RT_M_HP_MIXER|RT_M_SPK_MIXER|RT_M_MONO_MIXER); //unMute DAC to HP,Speaker,Mono Mixer
...
/* out path  */
int rt5621_set_outpath(u8 path)
{
    struct snd_soc_codec *codec = wm8580_codec;

	//printk(KERN_INFO "%s out_path:%d \n", __func__, path );

    switch(path)
    {
        case RT5621_NONE :
            printk(KERN_INFO "%s RT5621_NONE \n", __func__);
            rt5621_AudioOutEnable(codec, RT_WAVOUT_SPK, true);
            rt5621_AudioOutEnable(codec, RT_WAVOUT_HP, true);
            break;
        case RT5621_SPK :
            printk(KERN_INFO "%s RT5621_SPK \n", __func__);           
            rt5621_AudioOutEnable(codec, RT_WAVOUT_SPK, false);
            rt5621_AudioOutEnable(codec, RT_WAVOUT_HP, true);
            break;
        case RT5621_HP :
            printk(KERN_INFO "%s RT5621_HP \n", __func__);            
            rt5621_AudioOutEnable(codec, RT_WAVOUT_SPK, true);
            rt5621_AudioOutEnable(codec, RT_WAVOUT_HP, false);
            break;
        default :
            break;
    }
...
static struct wm8580_priv *wm8580;
