...
static int wm8960_probe(struct platform_device *pdev)
{
	...
	snd_soc_write(codec, WM8960_POWER2, 0x60); //LOUT1,ROUT1 enable
	snd_soc_write(codec, WM8960_APOP2, 0);
...
void wm8960_AudioOutEnable(struct snd_soc_codec *codec, int hp, int sp)
{
/* OUT1 - Headphones */    
	snd_soc_write(codec, WM8960_LOUT1, hp | 0x0100);
	snd_soc_write(codec, WM8960_ROUT1, hp | 0x0100);

/* OUT2 - Speakers */    
	snd_soc_write(codec, WM8960_LOUT2, sp | 0x0100);
	snd_soc_write(codec, WM8960_ROUT2, sp | 0x0100);
}

/* out path  */
int wm8960_set_outpath(u8 path)
{
    ...
    {
        case WM8960_NONE :
            printk(KERN_INFO "%s WM8960_NONE \n", __func__);
			wm8960_AudioOutEnable(codec, 0x0, 0x0);
            break;
        case WM8960_HP :
            printk(KERN_INFO "%s WM8960_HP \n", __func__);
            wm8960_AudioOutEnable(codec, 0x17f, 0x0);
            break;
        case WM8960_SPK :
            printk(KERN_INFO "%s WM8960_SPK \n", __func__);
            wm8960_AudioOutEnable(codec, 0x0, 0xff);
            break;
        default :
            break;
    }
...
EXPORT_SYMBOL(wm8960_set_outpath);
