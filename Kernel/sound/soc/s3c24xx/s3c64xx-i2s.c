...
int s5p_i2s_is_clk_enabled(void)
{
	return (!audio_clk_gated);
}
EXPORT_SYMBOL(s5p_i2s_is_clk_enabled);
...
static void s3c64xx_iis_dai_init(struct snd_soc_dai *dai)
{
	...
	dai->playback.channels_min = 1;	/* I2S dai : match Playback channels_min value with wm8580_dai(@wm8580.c) */
	dai->playback.channels_max = 2;
	dai->playback.rates = S3C64XX_I2S_RATES;
	dai->playback.formats = S3C64XX_I2S_FMTS;
	dai->capture.channels_min = 1;	/* I2S dai : fix channel_min value to enable Mono recording */
...
static __devinit int s3c64xx_iis_dev_probe(struct platform_device *pdev)
{
	...
	struct snd_soc_dai *dai;
	struct resource *res;
	struct clk *fout_epll, *mout_epll, *mout_audio, *mout_audss;
	unsigned long base;
	unsigned int  iismod;
	int ret;
	...
	/* Audio Clock
	 * fout_epll >> mout_epll >> mout_audio >> sclk_audio
	 * fout_epll >> mout_audss >> bus_clk(iis_cclk)
	 * fout_epll >> mout_audss >> mout_i2sa >> iis_clk(iis_pclk)
	 */
	...
	if (pdev->id == 0) {
		...
		if (IS_ERR(i2s->iis_pclk)) {
			dev_err(&pdev->dev, "failed to get iis_clock\n");
