...
static void s3c_idma_ctrl(int op)
{
	...
	switch (op) {
		case LPAM_DMA_START:
			...
			val |= 0x000F0000;     /* Clear LVL INT Flag */
...
#if 0
	printk("-iDmaAddr=@%x Total=%lubytes PrdSz=%u #Prds=%u\n",
...
static int s3c_idma_copy(struct snd_pcm_substream *substream, int channel,
		         ...
		         void __user *buf, snd_pcm_uframes_t count)
{
	...
	int frame_bytes = frames_to_bytes(runtime, count);

	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {	/* Playback only */
		if(count == runtime->buffer_size)
			ring_buf_index = (ring_buf_index + 1) % CONFIG_ANDROID_BUF_NUM;

		else if(count != runtime->buffer_size) {
			period_index += count;
		...
		if (copy_from_user(hwbuf, buf, frame_bytes))
			return -EFAULT;
	}
...
static irqreturn_t s3c_iis_irq(int irqno, void *dev_id)
{
	...
	if (val) {
		/* for Test
		if (((iisahb >> 20) & 0xF) != ((val >> 16) & 0xF)) {
			printk("## Wrong LVL INT = %X\n", (iisahb >> 20) & 0xF);
		}
		*/
		iisahb |= val;
		iisahb |= 0x000F0000;	/* Clear All LVL INT */
