...
static int s3c_wrpdma_copy(struct snd_pcm_substream *substream, int channel,
		        ...
		        void __user *buf, snd_pcm_uframes_t count)
{
	...
	if (platform->pcm_ops->copy)
		return platform->pcm_ops->copy(substream, channel, pos, buf, count);
	else
		return 0;
}
