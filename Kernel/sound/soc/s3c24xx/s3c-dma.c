...
static const struct snd_pcm_hardware s3c_dma_hardware = {
	...
	.channels_max		= 2,
	//SDKIM .buffer_bytes_max   = 128 * 1024,
...
static int s3c_dma_copy(struct snd_pcm_substream *substream, int channel,
		        ...
		        void __user *buf, snd_pcm_uframes_t count)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		if (copy_to_user(buf, hwbuf, frame_bytes))
			return -EFAULT;
	}
