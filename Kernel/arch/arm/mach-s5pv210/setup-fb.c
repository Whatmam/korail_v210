...
int s3cfb_backlight_onoff(struct platform_device *pdev, int onoff)
{
	...
	//printk("[MMM] s3cfb_backlight_onoff");
...
{
// jay 140109 - QT Backlight control - delete
	//int wtcon;
	//int pwm_freq = 145;
	//int pwm_level = 75;
	//printk("[mmm] s3cfb_backlight_on");
...
{
	//printk("[mmm] s3cfb_backlight_off");
#if 0
	s3c_gpio_setpin(S5PV210_GPD0(1), 0);
	gpio_free(S5PV210_GPD0(1));
#endif
	return 0;
}
...
{
#if defined(CONFIG_FB_S3C_LMS430HF18) || defined(CONFIG_FB_S3C_RT043T9T)
	int err;

	err = gpio_request(S5PV210_GPG3(6), "GPG36");
	
	if (err)
		printk(KERN_ERR "#### failed to request GPG36 for LCD\n");

	s3c_gpio_setpull(S5PV210_GPG3(6), S3C_GPIO_PULL_NONE);
	
	gpio_direction_output(S5PV210_GPG3(6), 0);
