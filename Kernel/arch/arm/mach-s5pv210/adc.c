...
#define ADCCON0_RES_10          (0x0<<16)
#define ADCCON0_RES_12          (0x1<<16)
#define ADCCON0_ECFLG           (1<<15)
#define ADCCON0_PRSCEN          (1<<14)
#define ADCCON0_START           (1<<0)

static inline unsigned int s3c_adc_convert(void)
{
/// hjkim 130529
///	unsigned int adc_return[5] = {0};
	unsigned int adc_return = 0;
	unsigned long data0;
	unsigned long data1;
	unsigned long ad_con;
///	int i = 0;
...
///			adc_return[i] = data1 & S3C_ADCDAT0_XPDATA_MASK_12BIT;
			adc_return = data1 & S3C_ADCDAT0_XPDATA_MASK_12BIT;
		else
///			adc_return[i] = data1 & S3C_ADCDAT0_XPDATA_MASK;
			adc_return = data1 & S3C_ADCDAT0_XPDATA_MASK;

///		if( i > 0 ) {
///			if ( (adc_return[i-1] & 0xf00) != (adc_return[i] & 0xf00) ) i = -1;
///		}
///		i++;
///	} while( i != 5 );
...
#endif
// hjkim 130529
	writel(readl(base_addr + S3C_ADCCON) & ~(1<<17), base_addr + S3C_ADCCON);
