...
unsigned short Korail_Dip_Switch(void)
{
        u16 *hw_dip_addr;
...
static void init_gpio_set(void)
{
	...
	//i2c2 sda
	gpio = S5PV210_GPD1(4);
	gpio_request(gpio, "GPD14");
	s3c_gpio_cfgpin(gpio, S3C_GPIO_SFN(0));
	gpio_free(gpio);
	
	//i2c2 sda
	gpio = S5PV210_GPD1(5);
	gpio_request(gpio, "GPD15");
	s3c_gpio_cfgpin(gpio, S3C_GPIO_SFN(0));
	gpio_free(gpio);
#endif
...
static struct s3c_platform_fb rt043t9t_fb_data __initdata = {
        .hw_ver = 0x62,
        .nr_wins = 5,
        .default_win = CONFIG_FB_S3C_DEFAULT_WINDOW,
        .swap = FB_SWAP_WORD | FB_SWAP_HWORD,
...
#ifdef CONFIG_FB_S3C_RT043T9T
        s3cfb_set_platdata(&rt043t9t_fb_data);
#endif
#else
#ifdef CONFIG_FB_S3C_LMS430HF18

// mkj 190108 LCD Add
	if((Korail_Dip_Switch() & 0x0002) == 0x0002)
	{
		s3cfb_set_platdata(&rt043t9t_fb_data);
	}
	else
