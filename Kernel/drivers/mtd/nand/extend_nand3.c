...
static struct mtd_info *extend_nand3_mtd = NULL;
 ...
 */
static void extend_nand3_hwcontrol(struct mtd_info *mtd, int cmd, unsigned int ctrl)
{
	struct nand_chip *chip = mtd->priv;

	if (ctrl & NAND_CTRL_CHANGE) {
		unsigned long addr = nand3_cwe_base;
		unsigned char bits;
 ...
 */
static int __init extend_nand3_init(void)
{
...
if(hwid_val == 0x0a){
	...
	extend_nand3_mtd = kmalloc(sizeof(struct mtd_info) + sizeof(struct nand_chip), GFP_KERNEL);
	if (!extend_nand3_mtd) {
	...
	this = (struct nand_chip *)(&extend_nand3_mtd[1]);

	/* Initialize structures */
	memset(extend_nand3_mtd, 0, sizeof(struct mtd_info));
	memset(this, 0, sizeof(struct nand_chip));

	/* Link the private data with the MTD structure */
	extend_nand3_mtd->priv = this;
	extend_nand3_mtd->owner = THIS_MODULE;


    nand3_cs_base= ioremap(KORAIL_DIV_Y3_EXT_ND_NCS2, 0x0040);
    nand3_cwe_base= ioremap(KORAIL_DIV_Y5_EXT_ND_CWE1, 0x0040);

	//NFCONT_REG &= ~NFCONT_WP;
	
	/* insert callbacks */
	this->IO_ADDR_R = (void *)nand3_cs_base;
	this->IO_ADDR_W = (void *)nand3_cs_base;
	this->cmd_ctrl = extend_nand3_hwcontrol;
	this->chip_delay = 50;
	this->ecc.mode = NAND_ECC_SOFT;

	/* Scan to find existence of the device */
	if (nand_scan(extend_nand3_mtd, 1)) {
		kfree(extend_nand3_mtd);
		return -ENXIO;
	}
	/* Register the partitions */
	add_mtd_partitions(extend_nand3_mtd, partition_info, NUM_PARTITIONS);
}
	/* Return happy */
	return 0;
}

module_init(extend_nand3_init);

/*
 * Clean up routine
 */
static void __exit extend_nand3_cleanup(void)
{
	/* Unregister the device */
	del_mtd_device(extend_nand3_mtd);

	/* Free the MTD device structure */
	kfree(extend_nand3_mtd);
}

module_exit(extend_nand3_cleanup);
