...
unsigned short Dip_Hw_Read(void)
{
        u16 *hw_dip_addr;
        unsigned short dip_read = 0;
        hw_dip_addr = ioremap(DIV_Y1_DIP , 0x0040);

        dip_read =  __raw_readw(hw_dip_addr);
	dip_read &= 0x000f;
        printk("Read Dip Setting = 0x%4x\r\n", dip_read);
        iounmap(hw_dip_addr);
...
static int s3cfb_probe(struct platform_device *pdev)
{
...
#if defined (CONFIG_FB_S3C_LMS430HF18)
	if((Dip_Hw_Read() & 0x0002) == 0x0002)
	{
		printk("[MMM] RT043T9T New LCD Reg Set\r\n");
		s3cfb_set_lcd_info_rt043t9t(fbdev);
	}
	else
	{
		printk("[MMM] LMS430HF18 Old LCD Reg Set\r\n");
		s3cfb_set_lcd_info_lms430hf18(fbdev);
	}
	...
	if (pdata->backlight_on)
		pdata->backlight_on(pdev);
#elif defined(CONFIG_FB_S3C_LTE480WV)  || defined(CONFIG_FB_S3C_LB070WV6) || defined(CONFIG_FB_S3C_LMS430HF18) || defined(CONFIG_FB_S3C_RT043T9T)
	if (pdata->backlight_on)
		pdata->backlight_on(pdev);
#endif
...
#if defined (CONFIG_FB_S3C_LTE480WV)  || defined(CONFIG_FB_S3C_LB070WV6) || defined(CONFIG_FB_S3C_LMS430HF18) || defined(CONFIG_FB_S3C_RT043T9T)
	if (pdata->backlight_off)
		pdata->backlight_off(pdev);
#endif
