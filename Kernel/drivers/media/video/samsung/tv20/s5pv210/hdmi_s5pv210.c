...
/*
	...
	writel(0x2c, hdmi_base + S5P_ASP_CHCFG2);
	writel(0x3e, hdmi_base + S5P_ASP_CHCFG3);

	writel((readl(hdmi_base + S5P_ASP_CON) &
		~(1<<7 | 3<<5 | 1<<4 | 0xf<<0))
			| (0<<7 | 0<<5 | 1<<4 | 0x7<<0),
		hdmi_base + S5P_ASP_CON);

	writel((readl(hdmi_base + S5P_ACR_CON) &
		~(3<<3 | 7<<0)) | (0<<3 | 4<<0),
		hdmi_base + S5P_ACR_CON);
