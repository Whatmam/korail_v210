...
static u32 sec_g2d_reserved_phys_addr = 0;
static u32 sec_g2d_reserved_size = 0;

static u32 sec_g2d_src_phys_addr = 0;
static u32 sec_g2d_src_virt_addr = 0;
static u32 sec_g2d_src_size      = 0;

static u32 sec_g2d_dst_phys_addr = 0;
static u32 sec_g2d_dst_virt_addr = 0;
static u32 sec_g2d_dst_size      = 0;
...
char *           time_watch_name[NUM_OF_STEP];
struct timeval   time_watch[NUM_OF_STEP];
int              time_watch_index = 0;
...
long sec_g2d_get_time_dur(struct timeval * time_before, struct timeval * time_after)
{
	...
	//timeval_to_ns
	/*
	if (time_after->tv_usec - time_before->tv_usec < 0)
	{
	...
	*/
	...
	/*
	if (time_before->tv_usec - time_after->tv_usec < 0)
	{
		msec = 1000000 + time_before->tv_usec - time_after->tv_usec;
		sec  = time_before->tv_sec - time_after->tv_sec - 1;
	}
	else
	{
		msec = time_before->tv_usec - time_after->tv_usec;
		sec  = time_before->tv_sec - time_after->tv_sec;
	}
	*/
...
{
	do_gettimeofday(&time_watch[ time_watch_index]);

	time_watch_name[time_watch_index] = name;
	time_watch_index++;
	...
	if(flag_print == 1)
	{
		...
		// 1 has no meaning..
		if(time_watch_index <= 0)
		{
			time_watch_index = 0;
			return;
		}

		total_dur = sec_g2d_get_time_dur(&time_watch[0], &time_watch[time_watch_index - 1]);

		for(i = 1; i < time_watch_index; i++)
		{			
			time_watch_dur = sec_g2d_get_time_dur(&time_watch[i-1], &time_watch[i]);
			/*
...
u32 sec_g2d_check_fifo_stat_wait(void)
{
	...
	while((!(__raw_readl(sec_g2d_base + FIFO_STAT_REG) & 0x1)) && (cnt > 0)){
		cnt--;
		msleep_interruptible(2);
	}

	if(cnt <= 0){
		__raw_writel(1, sec_g2d_base +FIFO_STAT_REG);
		return -1;
	}
...
u32 sec_g2d_set_src_img(g2d_rect * rect, g2d_flag * flag)
{
	u32 data    = 0;
	u32 blt_cmd = 0;
	...
	{
		// select source
		__raw_writel(G2D_SRC_SELECT_R_USE_FG_COLOR, sec_g2d_base + SRC_SELECT_REG);

		// foreground color
		__raw_writel(flag->src_color, sec_g2d_base + FG_COLOR_REG);		
	}
	else
	{
		// select source
		...
		__raw_writel(rect->phys_addr,   sec_g2d_base + SRC_BASE_ADDR_REG);

		// set stride
		data = get_stride_from_color_space(rect->color_format);
		__raw_writel(rect->full_w * data, sec_g2d_base + SRC_STRIDE_REG);
...
u32 sec_g2d_set_dst_img(g2d_rect * rect)
{
	...
	__raw_writel(rect->phys_addr,   sec_g2d_base + DST_BASE_ADDR_REG);

	// set stride
	data = get_stride_from_color_space(rect->color_format);
	__raw_writel(rect->full_w * data, sec_g2d_base + DST_STRIDE_REG);
...
u32 sec_g2d_set_clip_win(g2d_rect * rect)
{
	...
	if(rect->w != 0 && rect->h != 0)
	{
		...
		__raw_writel((rect->y << 16) | (rect->x), sec_g2d_base + CW_LEFT_TOP_REG);
		__raw_writel(((rect->y + rect->h) << 16) | (rect->x + rect->w), sec_g2d_base + CW_RIGHT_BOTTOM_REG);
	}
...
//	u32 data    = 0;
	...
	switch(flag->blue_screen_mode)
	{
		...
		case G2D_BLUE_SCREEN_TRANSPARENT :
			__raw_writel(flag->color_key_val, sec_g2d_base + BS_COLOR_REG);
		...
		case G2D_BLUE_SCREEN_WITH_COLOR :
			__raw_writel(flag->color_switch_val,     sec_g2d_base + BG_COLOR_REG);
			__raw_writel(flag->color_key_val, sec_g2d_base + BS_COLOR_REG);
...
u32 sec_g2d_set_pattern(g2d_rect * rect, g2d_flag * flag)
{
	...
	{	
		case G2D_THIRD_OP_PATTERN :
		{
			...
			__raw_writel(rect->phys_addr, sec_g2d_base + PAT_BASE_ADDR_REG);

			// set size of pattern image
			data =   ((rect->y + rect->h) << 16)
			       |  (rect->x + rect->w);
			__raw_writel(data, sec_g2d_base + PAT_SIZE_REG);

			// set stride
			data = get_stride_from_color_space(rect->color_format);
			__raw_writel(rect->full_w * data, sec_g2d_base + PAT_STRIDE_REG);

			// set color mode
			data = get_color_mode_from_color_space(rect->color_format);
			__raw_writel(data, sec_g2d_base + PAT_COLOR_MODE_REG);
			...
			//        | (params->w  - params->x);
			data =   (rect->y << 16) | rect->x;
			__raw_writel(data, sec_g2d_base + PAT_OFFSET_REG);
	...
	__raw_writel(data, sec_g2d_base + THIRD_OPERAND_REG);
	...
	__raw_writel(data, sec_g2d_base + ROP4_REG);
	...
	if(flag->mask_mode == TRUE)
	{
		...
		__raw_writel(rect->phys_addr,     sec_g2d_base + MASK_BASE_ADDR_REG);
		__raw_writel(rect->full_w * data, sec_g2d_base + MASK_STRIDE_REG);
...
u32 sec_g2d_set_alpha(g2d_flag * flag)
{
	u32 blt_cmd = 0;
//	u32 data    = 0;
	...
	if(flag->alpha_val <= G2D_ALPHA_VALUE_MAX) // < 255
	{
		...
		if(flag->alpha_val != 0)
			__raw_writel((flag->alpha_val & 0xff), sec_g2d_base + ALPHA_REG);
		else
			__raw_writel(0xff, sec_g2d_base + ALPHA_REG);	
	}
	/*
	else if (fading)
	{
		...
		__raw_writel((flag->alpha_val & 0xff) << G2D_BLT_CMD_R_ALPHA_BLEND_FAD_OFFSET, sec_g2d_base + ALPHA_REG);) {
	}
	*/
...
static int sec_g2d_init_regs(g2d_params *params)
{
	...
	/* source image */	
	blt_cmd |= sec_g2d_set_src_img(src_rect, flag);
	/* destination image */		
	...
	/* rop & alpha blending */
	blt_cmd |= sec_g2d_set_alpha(flag);
...
static void sec_g2d_rotate_with_bitblt(g2d_params *params)
{
	...
	// AP team
	__raw_writel(G2D_INTEN_R_CF_ENABLE, sec_g2d_base + INTEN_REG);

	__raw_writel(0x7, sec_g2d_base + CACHECTL_REG);

	__raw_writel(G2D_BITBLT_R_START, sec_g2d_base + BITBLT_START_REG);
}
...
{	
...
#if 0
	{
		...
		temp[i++] = __raw_readl(sec_g2d_base+SRC_COLORKEY_CTRL_REG);
		name[i] = SRC_COLORKEY_DR_MIN_REG;
		temp[i++] = __raw_readl(sec_g2d_base+SRC_COLORKEY_DR_MIN_REG);
		name[i] = SRC_COLORKEY_DR_MAX_REG;
		temp[i++] = __raw_readl(sec_g2d_base+SRC_COLORKEY_DR_MAX_REG);
		name[i] = DST_COLORKEY_CTRL_REG;
		temp[i++] = __raw_readl(sec_g2d_base+DST_COLORKEY_CTRL_REG);
		name[i] = DST_COLORKEY_DR_MIN_REG;
		temp[i++] = __raw_readl(sec_g2d_base+DST_COLORKEY_DR_MIN_REG);
		name[i] = DST_COLORKEY_DR_MAX_REG;
		temp[i++] = __raw_readl(sec_g2d_base+DST_COLORKEY_DR_MAX_REG);
...
irqreturn_t sec_g2d_irq(int irq, void *dev_id)
{
	...
	/*
	if((__raw_readl(sec_g2d_base + INTC_PEND_REG) & G2D_INTC_PEND_R_INTP_CMD_FIN) == 0)
	{
...
static int sec_g2d_mmap(struct file* filp, struct vm_area_struct *vma) 
{
	...
	pageFrameNo = __phys_to_pfn(sec_g2d_src_phys_addr);
	
	if(remap_pfn_range(vma, vma->vm_start, pageFrameNo, sec_g2d_reserved_size, vma->vm_page_prot))
	{
...
static int sec_g2d_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		case G2D_GET_MEMORY :
		{
			copy_to_user((unsigned int *)arg, &sec_g2d_reserved_phys_addr, sizeof(unsigned int));	
			return 0;
			break;
		}
		case G2D_GET_MEMORY_SIZE :
		{
			copy_to_user((unsigned int *)arg, &sec_g2d_reserved_size, sizeof(unsigned int));
			return 0;
			break;
		}
	...
	int flag_op_succed = 1;
	
	if(!(file->f_flags & O_NONBLOCK))
	{       
		if(wait_event_interruptible_timeout(waitq_g2d, (in_use == 0), msecs_to_jiffies(G2D_TIMEOUT)) == 0)
		{
...
int sec_g2d_probe(struct platform_device *pdev)
{
	...
	{
		/*
		sec_g2d_clock = clk_get(&pdev->dev, "g2d");
	...
	sec_g2d_reserved_phys_addr = s3c_get_media_memory_bank(S3C_MDEV_G2D, 0);
	if (sec_g2d_reserved_phys_addr == 0)
	{
		printk(KERN_ERR "fimg2d: failed to s3c_get_media_memory_bank !!! \n");
		return -ENOENT;
	}

	sec_g2d_reserved_size = s3c_get_media_memsize_bank(S3C_MDEV_G2D, 0);

	sec_g2d_src_phys_addr = sec_g2d_reserved_phys_addr;
	sec_g2d_src_virt_addr = (u32)phys_to_virt(sec_g2d_src_phys_addr);
	sec_g2d_src_size      = PAGE_ALIGN(sec_g2d_reserved_size >> 1);

	sec_g2d_dst_phys_addr = sec_g2d_src_phys_addr + sec_g2d_src_size;
	sec_g2d_dst_virt_addr = sec_g2d_src_virt_addr + sec_g2d_src_size;
	sec_g2d_dst_size      = PAGE_ALIGN(sec_g2d_reserved_size - sec_g2d_src_size);
