...
static int mfc_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	phy_size = (unsigned long)(s3c_get_media_memsize_bank(S3C_MDEV_MFC0, 1) - firmware_size
			+ s3c_get_media_memsize_bank(S3C_MDEV_MFC1, 1));
...
static int mfc_probe(struct platform_device *pdev)
{
	 ...
	 */
	mfc_port0_base_paddr = s3c_get_media_memory_bank(S3C_MDEV_MFC0, 1);
	...
	mfc_port1_base_paddr = s3c_get_media_memory_bank(S3C_MDEV_MFC1, 1);
