...
#if 1	// KSSON20100811(START)
#include <plat/gpio-cfg.h>
#include <linux/gpio.h>
#endif	// KSSON20100811(END)
...
#if 1	// KSSON20100811(START)
static struct timer_list pbtn_timer;	
#endif	// KSSON20100811(END)
...
#if 0	// KSSON20100811 : SMDKV210
		keymask[col] = rval; 
#else	// KSSON20100811(START)
		keymask[col] = rval & 0x03;
#endif	// KSSON20100811(END)
	}
...
#if 1	// KSSON20100811(START)
#include <linux/delay.h>
static void pbtn_timer_handler(unsigned long data){
	unsigned long flags;
	static ostat = 0;
	int stat;
	...
	stat = gpio_get_value(S5PV210_GPH0(1));


	if(!stat){
		if(ostat){
			printk("\t\t@@@ Adios..\n");
			mdelay(1000);/* Wait 1000msec */
			gpio_set_value(S5PV210_GPH0(0), 0);/* POwer OFF */
		}
	}

	local_irq_restore(flags);
	ostat = stat;
	mod_timer(&pbtn_timer, jiffies + HZ / 10);/* 100msec */
}

static int mvkey_check(int keyval){
	switch(keyval){
		case 9:		/* up */
		case 10:	/* menu */
		case 12:	/* right */
...
static void keypad_timer_handler(unsigned long data)
{
	...
	for (col=0; col < KEYPAD_COLUMNS; col++) {
		...
		//printk("keypad_timer_handler is called prevmask 0x%08x keypad 0x%08x\n", prevmask[col], keymask[col]);
	}

	if (restart_timer) {
		//printk("mod_timer is called\n");
...
{
	//printk("s3c_keypad_isr is called");
...
#else	// KSSON20100811(START)
	s3c_setup_keypad_cfg_gpio(2, KEYPAD_COLUMNS);
#endif	// KSSON20100811(END)
#endif
	writel(KEYIFCOL_CLEAR, key_base+S3C_KEYIFCOL);
...
#if 1	// KSSON20100811(START)
    init_timer(&pbtn_timer);
    pbtn_timer.function = pbtn_timer_handler;
    pbtn_timer.expires = jiffies + HZ / 10;
    add_timer(&pbtn_timer);
#endif	// KSSON20100811(END)
