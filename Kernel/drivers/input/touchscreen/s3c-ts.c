...
#define TOUCH_CAL_OVER_NUMBER		100
#define first_igno_data		10
...
long dmy_xdat = 0, dmy_ydat = 0;

int first_igno_count = 0;
...
static void touch_timer_fire(unsigned long data)
{
	...
	if (updown) 
	{
		if (ts->count) 
		{
			...
			tmp_x = (dmy_xdat + ts->xp) / 2;
			tmp_y = (dmy_ydat + ts->yp) / 2;

			if(dmy_xdat > tmp_x)
			{
				sum_x = dmy_xdat - tmp_x;
			}
			else
			{
				sum_x = tmp_x - dmy_xdat;
			}

			if(dmy_ydat > tmp_y)
			{
				sum_y = dmy_ydat - tmp_y;
			}
			else
			{
				sum_y = tmp_y - dmy_ydat;
			}

			if(first_igno_count > first_igno_data)
			{

				if((sum_x < TOUCH_CAL_OVER_NUMBER) && (sum_y < TOUCH_CAL_OVER_NUMBER))
				{
