...
dm9000_rx(struct net_device *dev)
{
	...
	do {
		...
		if(rxbyte & (1<<2)){
			printk("[%s] eth down:FIFO Overflow", __func__);
			rx_errcnt++;
			if(rx_errcnt > 1) return 1;
		}
		rxbyte = ior(db, 0x06);
		//printk("[%s] RXSR: %x\n", __func__, rxbyte);
...
dm9000_phy_read(struct net_device *dev, int phy_reg_unused, int reg)
{
	...
	dm9000_msleep(db, MDELAY);		/* Wait read complete */
		 ...
		 int phyaddr_unused, int reg, int value)
{
	...
	dm9000_msleep(db, MDELAY);		/* Wait write complete */
